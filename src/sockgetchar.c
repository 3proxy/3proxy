...
int sockgetcharcli(struct clientparam * param, int timeosec, int timeousec){
	...
	param->clioffset = param->cliinbuf = 0;
	if ((len = sockrecvfrom(param, param->clisock, (struct sockaddr *)&param->sincr, param->clibuf, param->clibufsize, timeosec*1000 + timeousec))<=0) return EOF;
	param->cliinbuf = len;
	param->clioffset = 1;
...
int sockfillbuffcli(struct clientparam * param, unsigned long size, int timeosec){
	...
	size -= param->cliinbuf;
	if((len = sockrecvfrom(param, param->clisock, (struct sockaddr *)&param->sincr, param->clibuf + param->cliinbuf, (param->clibufsize - param->cliinbuf) < size? param->clibufsize - param->cliinbuf:size, timeosec*1000)) > 0){
		param->cliinbuf += len;
	}
...
int sockfillbuffsrv(struct clientparam * param, unsigned long size, int timeosec){
	...
	size -= param->srvinbuf;
	if((len = sockrecvfrom(param, param->remsock, (struct sockaddr *)&param->sinsr, param->srvbuf + param->srvinbuf, (param->srvbufsize - param->srvinbuf) < size? param->srvbufsize - param->srvinbuf:size, timeosec*1000)) > 0){
		param->srvinbuf += len;
		param->nreads++;
...
int sockgetcharsrv(struct clientparam * param, int timeosec, int timeousec){
	...
	param->srvoffset = param->srvinbuf = 0;
	if ((len = sockrecvfrom(param, param->remsock, (struct sockaddr *)&param->sinsr, param->srvbuf, param->srvbufsize, timeosec*1000 + timeousec))<=0) return EOF;
	param->srvinbuf = len;
	param->srvoffset = 1;
	param->nreads++;
	param->statssrv64 += len;
